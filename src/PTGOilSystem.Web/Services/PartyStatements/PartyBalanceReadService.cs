...
public sealed class PartyBalanceReadService : IPartyBalanceReadService
{
        ...
        CancellationToken ct)
    {
        ...
        async Task AddAsync(
            ...
            IQueryable<(int Id, string Name)> query)
        {
            var ids = events.Where(e => e.PartyType == type).Select(e => e.PartyId).Distinct().ToArray();
            if (ids.Length == 0) return;
            var rows = await query.Where(row => ids.Contains(row.Id)).ToListAsync(ct);
            foreach (var row in rows) result[(type, row.Id)] = row.Name;
        }

        await AddAsync(PartyStatementPartyType.Customer,
            _db.Customers.AsNoTracking().Select(x => new ValueTuple<int, string>(x.Id, x.Name)));
        await AddAsync(PartyStatementPartyType.Supplier,
            _db.Suppliers.AsNoTracking().Select(x => new ValueTuple<int, string>(x.Id, x.Name)));
        await AddAsync(PartyStatementPartyType.ServiceProvider,
            _db.ServiceProviders.AsNoTracking().Select(x => new ValueTuple<int, string>(x.Id, x.Name)));
        await AddAsync(PartyStatementPartyType.Driver,
            _db.Drivers.AsNoTracking().Select(x => new ValueTuple<int, string>(x.Id, x.FullName)));
        await AddAsync(PartyStatementPartyType.Company,
            _db.Companies.AsNoTracking().Select(x => new ValueTuple<int, string>(x.Id, x.Name)));
        await AddAsync(PartyStatementPartyType.Sarraf,
            _db.Sarrafs.AsNoTracking().Select(x => new ValueTuple<int, string>(x.Id, x.Name)));
        await AddAsync(PartyStatementPartyType.Employee,
            _db.Employees.AsNoTracking().Select(x => new ValueTuple<int, string>(x.Id, x.FullName)));
        await AddAsync(PartyStatementPartyType.Partner,
            _db.Partners.AsNoTracking().Select(x => new ValueTuple<int, string>(x.Id, x.Name)));
